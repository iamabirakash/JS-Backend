...
const updateAccountDetails = asyncHandler(async(req, res) => {
    ...
    const user = await User.findByIdAndUpdate( //Model.findByIdAndUpdate(id, update, options, callback);
        req.user?._id,
        {
            $set: {
                fullName,
        ...
        {new: true} //this return the updated value
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $addFields: {
                ...
                isSubscribed: {
                    $cond: {
                        if: {$in: [req.user?._id, "$subscribers.subscriber"]}, //$in Operator: Checks if the value of userId is in the array "$subscribers.subscriber"
                        then: true,
                        else: false
                    }
                }
